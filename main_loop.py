def dF_in_out(iorb,nsteps_loop):
  import orbit,grid
  import numpy as np
  from math import floor
  from parameters import mi,qi
  ...
  if orbit.steps_orb[iorb-1]==1: return np.mean(dF_orb,axis=0)
  for it_orb in [0,orbit.steps_orb[iorb-1]-1]:
    ...
    #end for itr
    dF_orb[:,:]=dF_orb[:,:]+sgn*df0g_orb[:,:]*(mi/np.pi/2)**1.5/1.6022E-19
  #end for it_orb
      ...
      for i in range(3):
        ...
        if (mu_n<grid.f0_smu_max**2)and(vp_n<grid.f0_vp_max)and(vp_n>=-grid.f0_vp_max):
          ...
          if (idx==1)and(xgc=='xgc1')and(use_ff):
            #use field-line following to interpoloate F at integer planes
            tmp[0:2,0:2,:,:],gradParF_tmp=grid.gradParF_ff(node,imu,ivp,nsteps_loop,wmu,wvp)
            Br_l=Br_l+p[i]*Br
            Bz_l=Bz_l+p[i]*Bz
            Bphi_l=Bphi_l+p[i]*Bphi
            gradParF[:,:]=gradParF[:,:]+p[i]*gradParF_tmp/np.sqrt(2*mu*B)
          else:
            tmp[0:2,0:2,:,:]=grid.df0g[ivp:ivp+2,inode,imu:imu+2,:,:]
...
def copy_data(idx,nsteps_loop,iphi):
  ...
  if (xgc=='xgc1')and(idx==1):
    if use_ff:
      ...
      df0g_gpu,dFdpara_gpu=grid.gradParF_ff_gpu(B_gpu,iphi,nsteps_loop)
    else:
      ...
      df0g_gpu=cp.array((grid.df0g[:,:,:,iphim1,:]+grid.df0g[:,:,:,iphip1,:])/2.,dtype=cp.float64).ravel(order='C')
